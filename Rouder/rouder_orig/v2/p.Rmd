---
title             : "Is the Antisaccade Task a Unicorn Task For Measuring Cognitive Control?"
shorttitle        : "Antisaccade a Unicorn Task?"
...
\end{aligned}
\]
Here, the condition means $\nu_1$ (prosaccade) and $\nu_2$ (antisaccade) are allowed to vary resulting in a smaller residual variance.  Under Model ${\cal M}_2$, $\delta^2= `r round(del2[2],1)`$ms$^2$, which is much smaller than the original value under model $\calM_1$ of  `r round(del2[1],1)`.  Dividing reveals that the condition effects in Model ${\cal M}_2$ accounts for about `r round(100*(1-del2[2]/del2[1]),1)`% of the total variance in $\mu_{ij}$.  Indeed, there is a large difference in overall performance between the prosacade and antisaccade conditions.
...
These analyzes converge on the same conclusion: individual differences in performance in the prosaccade and antisaccade conditions reflect variation on a single factor.
