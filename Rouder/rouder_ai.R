...
# Scree plot
scree_data <- fa.parallel(df_numeric, fm = "ml", fa = "fa", 
...
if (!is.null(efa_2f_oblimin$Phi)) {
  ...
  if (abs(factor_cor) > 0.7) {
    cat("⚠️ High factor correlation (r =", round(factor_cor, 3), 
        ") suggests factors may not be distinct\n")
  } else if (abs(factor_cor) < 0.3) {
    cat("✓ Low factor correlation (r =", round(factor_cor, 3), 
...
get_fit_measures <- function(fit_object, model_name) {
  ...
  fits <- fitMeasures(fit_object, c("chisq", "df", "pvalue", "cfi", "tli", 
                                    "rmsea", "srmr", "aic", "bic"))
...
if (fit_1f@Fit@converged && fit_2f@Fit@converged) {
  ...
  cat("─────────────────────\n")
  cat("1. Statistical improvement (χ² diff test): p =", 
      ifelse(is.na(p_diff), "NA", 
             ifelse(p_diff < 0.001, "<0.001", round(p_diff, 3))), "\n")
  cat("2. Practical improvement:\n")
  cat("   - ΔCFI =", round(cfi_improvement, 3), 
      ifelse(cfi_improvement > 0.01, "✓", "✗"), "\n")
  cat("   - ΔRMSEA =", round(rmsea_improvement, 3), 
      ifelse(rmsea_improvement > 0.015, "✓", "✗"), "\n")
  cat("   - ΔBIC =", round(bic_improvement, 2), 
      ifelse(bic_improvement > 2, "✓", "✗"), "\n")
  cat("3. Factor correlation:", round(factor_cor_cfa, 3), 
  ...
  cat("Temporal (CIRCLES-SQUARES):", 
      round(cor(temporal_items)[1,2], 3), "\n")
  cat("Inspection (CIRCLES-SQUARES):", 
...
score_vars <- c("temporal_circles_z", "temporal_squares_z", 
...
cat("- Observed correlations are attenuated by factor of:", 
    round(sqrt(alpha_inspection$total$raw_alpha), 3), "\n")
cat("- To detect r = 0.30, you need observed r =", 
    round(0.30 * sqrt(alpha_inspection$total$raw_alpha), 3), "\n")
cat("- Power is reduced by approximately", 
