...
accuracy_blue <- ifelse(pdi <= 500, 80, 80)
accuracy_green <- ifelse(pdi <= 500, 60, 60 + 0.01 * (pdi - 500))
accuracy_red <- ifelse(pdi <= 500, 50, 50 + 0.02 * (pdi - 500))
...
# Create the plot
ggplot(data) +
  geom_line(aes(x = PDI, y = BlueLine), color = "blue") +
  geom_line(aes(x = PDI, y = GreenLine), color = "green") +
  geom_line(aes(x = PDI, y = RedLine), color = "red") +
  labs(title="Possible dynamics of the incremental relationship between PDI and antisaccade accuracy",
  ...
  annotate("text", x = 500, y = 55, label = "Black dashed vertical line represents the conventional value\nafter which the influence of the attentional blink disappears", 
           size = 3, angle = 270, hjust = 0) +
  theme_apa() +
